...
## 简介
本文件面向“新浪数据采集器”的实现进行系统化技术文档梳理，重点围绕以下目标展开：
...
- SinaCollector：实现 BaseCollector 的接口，对接新浪财经的实时行情接口，完成字符串解析与字段映射。
...
- API 调用：
  ...
  - 列表/K线/分时/盘口：当前版本仅记录日志提示不支持。
- 数据解析与字段映射：
  - 解析响应字符串，按逗号分割提取字段，进行类型转换与默认值处理。
  - 计算涨跌额与涨跌幅，构造标准字典，包含时间戳与市场标识。
...
- 特殊限制：列表、K线、分时、盘口目前仅尝试 eastmoney，避免调用不支持的接口。
...
- 超时与重试：当前采集器设置统一超时，未实现指数退避重试；建议在 CollectorManager 或采集器内部增加有限重试与退避策略。
...
## 故障排查指南
- 接口不可用：当 EastMoney 不可用时，CollectorManager 会自动回退到 Sina；若两者均失败，返回空值。检查网络连通性与目标站点状态。
- 数据为空：Sina 的 fetch_quote 在字段不足或响应格式异常时返回空值；检查 symbol 前缀与市场匹配。
...
## 结论
- SinaCollector 作为备用采集器，实现了最小可用的实时行情获取能力，适合在主数据源不可用时进行回退。
...
### 与东方财富采集器的差异与优势
- 接口覆盖度：SinaCollector 当前仅实现实时行情；EastMoneyCollector 实现了列表、K线、分时、盘口等完整生态。
- 数据质量：EastMoney 提供更丰富的字段与更稳定的 JSON 接口，解析成本更低；Sina 使用字符串解析，需自行拆分与校验。
- 可靠性：EastMoney 接口更成熟，错误率更低；Sina 在字段缺失或格式异常时易返回空值。
- 优势：SinaCollector 作为回退方案，具备简单、稳定的特点，适合在主数据源异常时快速恢复服务能力。

章节来源
- [sina.py:62-79](file://backend/app/services/collector/sina.py#L62-L79)
- [eastmoney.py:39-185](file://backend/app/services/collector/eastmoney.py#L39-L185)

### 实际代码示例路径（不展示具体代码内容）
- 实时行情获取（SinaCollector）：[fetch_quote 实现:19-60](file://backend/app/services/collector/sina.py#L19-L60)
- 实时行情获取（EastMoneyCollector）：[fetch_quote 实现:23-37](file://backend/app/services/collector/eastmoney.py#L23-L37)
- 行情列表获取（EastMoneyCollector）：[fetch_quote_list 实现:39-99](file://backend/app/services/collector/eastmoney.py#L39-L99)
- K线获取（EastMoneyCollector）：[fetch_kline 实现:101-147](file://backend/app/services/collector/eastmoney.py#L101-L147)
- 分时获取（EastMoneyCollector）：[fetch_timeline 实现:149-185](file://backend/app/services/collector/eastmoney.py#L149-L185)
- 盘口获取（EastMoneyCollector）：[fetch_orderbook 实现:187-222](file://backend/app/services/collector/eastmoney.py#L187-L222)
- 故障转移（CollectorManager）：[自动回退逻辑:21-32](file://backend/app/services/collector/manager.py#L21-L32)
